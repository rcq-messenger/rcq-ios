...
if [[ "${CARGO_BUILD_TARGET:-}" =~ -ios(-sim|-macabi)?$ ]]; then
  ...
  # RCQ patch: full LTO emits LLVM bitcode .o files which break
  # `xcodebuild -create-xcframework` (it expects native Mach-O).
  # Trade size for being able to package the static lib into an
  # xcframework. Final binary grows by ~10-20MB which is fine for
  # an iOS app of this size.
  # export CARGO_PROFILE_RELEASE_LTO=fat
  # export CFLAGS="-flto=full ${CFLAGS:-}"
  # export CXXFLAGS="-flto=full ${CXXFLAGS:-}"

  # RCQ patch: -DOPENSSL_SMALL leaks via the global CFLAGS into the `ring`
  # build, which gates p256-nistz.c on !OPENSSL_SMALL. With it set, `ring`
  # ends up missing _ring_core_*_p256_point_mul_base_vartime at link time.
  # Disabling for now — final binary is a few hundred KB larger than ideal.
